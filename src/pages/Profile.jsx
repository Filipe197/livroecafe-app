...
function BadgeCard({ badge, unlocked }) {
  return (
    <div style={{
      ...
      {unlocked && (
        <div style={{ position: 'absolute', top: 6, right: 6, width: 8, height: 8, background: 'var(--gold)', borderRadius: '50%' }} />
      )}
    </div>
  )
}
...
export default function Profile() {
  ...
  const settingsItems = [
    { icon: '🔔', label: 'Notificações', content: (
      <div style={{ padding: '12px 0' }}>
        {['Novos livros adicionados', 'Lembretes de leitura', 'Promoções e ofertas'].map(item => (
          <div key={item} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '0.5px solid var(--border)' }}>
            ...
            <div style={{ width: 40, height: 22, background: 'var(--gold)', borderRadius: 11, position: 'relative' }}>
    ...
    { icon: '❓', label: 'Ajuda e suporte', content: (
      <div style={{ padding: '12px 0' }}>
        {[['📧 Contato', 'suporte@livroecafe.com.br'], ['🌐 Site', 'livroecafe.com.br'], ['📱 Versão', '1.0.0']].map(([l, v]) => (
    ...
    <div className="fade-in">
      <div style={{ padding: '20px 16px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 22 }}>Meu Perfil</h1>
          {!editing
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
          ...
          <div style={{ flex: 1 }}>
            {editing ? (
              ...
              <div style={{ fontFamily: 'var(--font-serif)', fontSize: 18, marginBottom: 2 }}>{displayName}</div>
            )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 20 }}>
          {[{ label: 'Lidos', value: booksRead }, { label: 'Lendo', value: reading }, { label: 'Favoritos', value: favorites.length }].map(s => (
        ...
        {streak > 0 && (
          <div style={{ marginBottom: 20, background: 'linear-gradient(135deg, rgba(212,90,58,0.15), rgba(232,150,50,0.15))', border: '0.5px solid rgba(212,90,58,0.3)', borderRadius: 12, padding: 14, display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ fontSize: 40, animation: streak >= 7 ? 'pulse 1.5s ease-in-out infinite' : 'none' }}>🔥</div>
            <div>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['unlocked', 'all'].map(t => (
                <button key={t} onClick={() => setBadgeTab(t)} style={{
                  ...
                  color: badgeTab === t ? '#0f0e0c' : 'var(--dim)',
              ...
              {allProgress.filter(p => p.progress_percent < 100 && p.progress_percent > 0).slice(0, 3).map(p => (
                <div key={p.id} onClick={() => navigate(`/read/${p.book_id}`)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 10, padding: 10, marginBottom: 8, cursor: 'pointer' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: 'var(--font-serif)', fontSize: 12, marginBottom: 5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.books?.title}</div>
        ...
        {settingsItems.map(item => (
          <div key={item.label}>
            <div onClick={() => setActiveSettings(activeSettings === item.label ? null : item.label)}
              ...
              <span style={{ color: 'var(--dim)', fontSize: 18, transition: 'transform 0.2s', display: 'inline-block', transform: activeSettings === item.label ? 'rotate(90deg)' : 'none' }}>›</span>
            </div>
            {activeSettings === item.label && <div style={{ paddingLeft: 30 }}>{item.content}</div>}
          </div>
        ))}

        <button onClick={signOut} style={{ width: '100%', marginTop: 24, marginBottom: 8, padding: '13px 0', background: 'none', border: '0.5px solid var(--border)', borderRadius: 10, color: '#c0564a', fontSize: 14 }}>
          Sair da conta
        </button>
      </div>
    </div>
