        ...
        {loading ? <Spinner /> : messages.length === 0 ? (
          ...
          return (
            <div key={msg.id} style={{ display: 'flex', gap: 8, flexDirection: isMe ? 'row-reverse' : 'row', alignItems: 'flex-end' }}>
              {!isMe && (
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--surface2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, flexShrink: 0 }}>{avatar}</div>
              )}
              <div style={{ maxWidth: '75%' }}>
                {!isMe && <div style={{ fontSize: 10, color: 'var(--dim)', marginBottom: 3, paddingLeft: 4 }}>{name}</div>}
                <div style={{
                  background: isMe ? 'var(--gold)' : 'var(--surface)',
                  color: isMe ? '#0f0e0c' : 'var(--text)',
      ...
      {user ? (
        <div style={{ padding: '10px 16px', borderTop: '0.5px solid var(--border)', display: 'flex', gap: 8, flexShrink: 0, background: 'var(--bg)' }}>
          ...
          <button onClick={send} disabled={!text.trim() || sending} style={{
            width: 40, height: 40, background: text.trim() ? 'var(--gold)' : 'var(--surface)',
...
export default function Clubs() {
  ...
  async function createClub() {
    ...
    setCreating(true)
    const { data } = await supabase.from('book_clubs').insert({
  ...
  if (activeClub) return <Chat club={activeClub} user={user} onBack={() => { setActiveClub(null); fetchClubs() }} />
  ...
  return (
    <div className="fade-in">
      <div style={{ padding: '20px 16px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {user && (
          <button onClick={() => setShowCreate(true)} style={{ background: 'var(--gold)', color: '#0f0e0c', border: 'none', borderRadius: 12, padding: '8px 14px', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>+ Criar</button>
        )}
      </div>
      ...
      {showCreate && (
        <div style={{ margin: '0 16px 16px', background: 'var(--surface)', border: '0.5px solid var(--gold)', borderRadius: 12, padding: 16 }}>
          ...
          <textarea value={newDesc} onChange={e => setNewDesc(e.target.value)} placeholder="Descrição (opcional)..." rows={2}
            style={{ width: '100%', background: 'var(--surface2)', border: '0.5px solid var(--border)', borderRadius: 8, padding: '10px 12px', color: 'var(--text)', fontSize: 14, outline: 'none', fontFamily: 'var(--font-sans)', marginBottom: 10, resize: 'none', boxSizing: 'border-box' }} />
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={createClub} disabled={!newName.trim() || creating} style={{ flex: 1, background: 'var(--gold)', color: '#0f0e0c', border: 'none', borderRadius: 8, padding: '10px 0', fontWeight: 500, fontSize: 13, cursor: 'pointer' }}>
          ...
          {clubs.map(club => {
            ...
            return (
              <div key={club.id} style={{ background: 'var(--surface)', border: `0.5px solid ${isMember ? 'var(--gold)' : 'var(--border)'}`, borderRadius: 12, padding: 14, marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => setActiveClub(club)} style={{ flex: 1, background: 'var(--surface2)', color: 'var(--text)', border: '0.5px solid var(--border)', borderRadius: 8, padding: '9px 0', fontSize: 13, cursor: 'pointer' }}>
                    💬 Chat
                  </button>
                  {isMember
